...
//   description: string;
//   href?: string;
//   isActive?: boolean; // Optional flag to filter active slides
// }
...
//           const activeSlides = res.data.filter(
//             (slide: CarouselSlide) => slide.isActive !== false,
//           );
...
//   if (loading) {
//     return (
//       <div className="w-full h-150 lg:h-187.5 bg-gray-900 flex items-center justify-center">
...
//   return (
//     <div className="relative w-full h-150 lg:h-187.5overflow-hidden bg-gray-900">
    ...
    <div className="relative w-full h-[90svh] min-h-[750px] lg:h-[100svh] lg:min-h-[800px] overflow-hidden bg-gray-900">
      ...
      <AnimatePresence>
        ...
        >
          <Image
            src={currentData.image}
            alt={currentData.title || "Carousel Slide"}
            fill
      ...
      <div className="absolute top-0 left-0 w-full z-40">
        ...
        {/* Search and Icons */}
        <div
          className={`relative flex justify-center lg:justify-end px-4 lg:px-12 mt-4 lg:mt-6 transition-opacity duration-300 ${
            isMenuOpen ? "opacity-0 pointer-events-none" : "opacity-100"
          }`}
        >
          <div className="flex items-center gap-4 flex-wrap justify-center">
      ...
      {/* Text Content */}
      {/* pb-24 added to keep content safe from the bottom edge */}
      <div className="absolute inset-0 flex flex-col justify-center items-end text-right z-30 px-6 md:px-12 lg:px-20 pointer-events-none pb-24 lg:pb-0 mt-8 lg:mt-0">
        <AnimatePresence mode="wait">
          <motion.div
            key={currentData.title + currentSlide}
            className="pointer-events-auto w-full max-w-sm md:max-w-xl lg:max-w-2xl"
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -50 }}
            transition={{ duration: 0.5 }}
