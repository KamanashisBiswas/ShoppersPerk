...
export default function SubCategoryManagement() {
  ...
  // Details State
  const [detailsItem, setDetailsItem] = useState<SubCategoryItem | null>(null);
  ...
  const fetchItems = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/sub-category`);
  ...
  const handleOpenModal = (item?: SubCategoryItem) => {
    if (userRole !== 'admin') {
  ...
  const handleViewDetails = (item: SubCategoryItem) => {
     setDetailsItem(item);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingItem
        ? `${API_BASE_URL}/sub-category/${editingItem.subCategoryId}`
        : `${API_BASE_URL}/sub-category`;
      ...
      toast.success(editingItem ? 'Sub-Category updated successfully' : 'Sub-Category created successfully');
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/sub-category/${itemToDelete}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                ...
                <th className="px-6 py-5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                    <div className="flex items-center gap-2"><MdCategory size={16} /> Parent Category</div>
                </th>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                    No sub-categories found. Add your first one above.
                  </td>
                </tr>
              ) : (
                items.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                        <div className="flex flex-col">
                            <span className="text-xs font-mono text-purple-400 mb-1">{item.subCategoryId}</span>
                    ...
                    <td className="px-6 py-4">
                        <span className="px-3 py-1 bg-white/5 rounded-lg text-sm text-gray-300 border border-white/10">
                            {item.parentCategory?.name || 'Unknown'}
                        </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                       <div className="flex items-center justify-end gap-2">
                           ...
                           <button 
                             onClick={() => confirmDelete(item.subCategoryId)}
      ...
      <Modal
        ...
        title={`${editingItem ? 'Edit' : 'Add'} Sub-Category`}
      >
        <form onSubmit={handleSubmit} className="space-y-5 text-gray-300">
            ...
            <div>
                <label className="block text-xs font-semibold uppercase text-gray-500 mb-1.5 ml-1">Sub-Category Name</label>
            ...
            <div>
                <label className="block text-xs font-semibold uppercase text-gray-500 mb-1.5 ml-1">Parent Category</label>
                <div className="relative">
                    <select 
                        value={formData.parentCategory} 
                        onChange={e => setFormData({...formData, parentCategory: e.target.value})}
                        className="w-full bg-[#1a1a1a] border border-white/10 focus:border-purple-500 rounded-xl px-4 py-3 outline-none transition-colors text-white appearance-none cursor-pointer"
                    >
            ...
            >
                {isSubmitting ? (
                    ...
                    <>
                        ...
                        <span>{editingItem ? 'Update Sub-Category' : 'Create Sub-Category'}</span>
                    </>
                )}
            </button>
        </form>
                ...
                <div className="grid grid-cols-1 gap-4">
                    <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                        <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                            <MdCategory /> Parent Category
                        </h4>
                        <p className="font-semibold text-purple-300 text-lg">{detailsItem.parentCategory?.name || 'Unknown'}</p>
                    </div>

                    <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                        <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                            <MdNumbers /> Sub-Category ID
                        </h4>
                        <p className="font-mono text-white text-lg">{detailsItem.subCategoryId}</p>
                    </div>
