...
export default function ProductManagement() {
  ...
  const fetchDependencies = async () => {
    try {
      ...
      if (catRes.ok) {
        const data: CategoryItem[] = await catRes.json();
        setCategories(data.map((item) => ({ id: item._id, name: item.name })));
      }
      if (brandRes.ok) {
        const data: BrandItem[] = await brandRes.json();
        setBrands(data.map((item) => ({ id: item._id, name: item.name })));
      }
      if (subCatRes.ok) {
        const data: SubCategoryItem[] = await subCatRes.json();
        setSubCategories(
          data.map((item) => ({ id: item._id, name: item.name })),
        );
      }
    } catch {
  ...
  const handleOpenFormModal = (item?: ProductItem) => {
    ...
    if (item) {
      ...
      if (item.variation && Array.isArray(item.variation)) {
        setVariations(
          item.variation.map((v: Variation) => ({
            size: v.size || "",
            color: v.color || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const v of variations) {
      if (!v.size && !v.color)
        return toast.error("Variation must have size or color");
      if (!v.basePrice || !v.stock)
        return toast.error("Variation price and stock are required");
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            Manage your catalog, inventory, and variations
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {userRole === "admin" && (
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-linear-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 rounded-xl transition-all shadow-lg hover:shadow-purple-500/25 active:scale-95 text-white font-medium"
            >
              ...
              <tr className="border-b border-white/5 bg-white/2">
                ...
                <th className="px-6 py-5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                  <div className="flex items-center gap-2">
                    <MdTitle size={16} /> Name & Ref
                  </div>
                </th>
                <th className="px-6 py-5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                  <div className="flex items-center gap-2">
                    <MdCategory size={16} /> Category/Brand
                  </div>
                </th>
                <th className="px-6 py-5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        ...
                        <div className="mt-2">
                          <span
                            className={`text-[10px] px-2 py-0.5 rounded-full border ${item.isActive ? "bg-green-500/10 text-green-500 border-green-500/20" : "bg-red-500/10 text-red-500 border-red-500/20"}`}
                          >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col text-sm text-gray-400 gap-1">
                        <span className="flex items-center gap-1">
                          <MdCategory size={14} className="text-gray-500" />{" "}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="block text-xs font-semibold uppercase text-gray-500 mb-1.5 ml-1">
                Product Name
              </label>
              <input
                type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-white/10">
            <div>
              <label className="block text-xs font-semibold uppercase text-gray-500 mb-3 ml-1">
                Banner Image
              </label>
              <div
          ...
          <div className="pt-4 border-t border-white/10">
            <div className="flex items-center justify-between mb-4">
              <label className="block text-xs font-semibold uppercase text-gray-500 ml-1">
                Variations
              </label>
              <button
                type="button"
            ...
            <div className="space-y-3">
              {variations.map((variation, idx) => (
                <div
                  key={idx}
                  className="grid grid-cols-12 gap-2 p-3 bg-white/5 rounded-xl items-end relative group"
                >
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleVariationChange(idx, "size", e.target.value)
                      }
                  ...
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleVariationChange(idx, "color", e.target.value)
                      }
                      className="w-full bg-black/30 border border-white/10 rounded-lg px-2 py-1.5 text-sm text-white focus:border-purple-500 outline-none"
                    />
                  </div>
                  <div className="col-span-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleVariationChange(idx, "basePrice", e.target.value)
                      }
                      className="w-full bg-black/30 border border-white/10 rounded-lg px-2 py-1.5 text-sm text-white focus:border-purple-500 outline-none"
                    />
                  </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              ...
              className="flex-1 py-3 rounded-xl bg-linear-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold shadow-lg shadow-purple-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSubmitting
                ? "Processing..."
                : editingItem
          ...
          <div className="space-y-8 text-gray-300">
            ...
            <div className="flex flex-col items-center justify-center p-6 bg-white/5 rounded-2xl border border-white/10">
              ...
              <div className="flex gap-3">
                <span
                  className={`px-3 py-1 rounded-full text-xs font-bold border ${detailsItem.isActive ? "bg-green-500/10 text-green-500 border-green-500/20" : "bg-red-500/10 text-red-500 border-red-500/20"}`}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                ...
                <p className="font-medium text-white">
                  {detailsItem.category?.name || "N/A"}
                </p>
                <p className="text-sm text-gray-400">
                  {detailsItem.subCategory?.name || "N/A"}
                </p>
              </div>
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2">
              ...
              <div className="bg-white/5 rounded-xl border border-white/10 overflow-hidden">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-white/5 text-gray-300">
                    {detailsItem.variation?.map((v: Variation, idx: number) => (
                      <tr key={idx} className="hover:bg-white/5">
                        <td className="px-4 py-2">{v.size || "-"}</td>
                        <td className="px-4 py-2">{v.color || "-"}</td>
                        <td className="px-4 py-2 font-mono text-purple-400 font-medium">
                          ${v.basePrice}
                        </td>
